            ...
            if( is_cart() ){
                ...
                if ( WC()->cart->is_empty() && !empty( $empty_cart_page_id ) ) {
                    ...
                    add_action('woolentor/builder/content', function () use ( $template_id ) {
                        include_once ( $this->get_template_part('cart-empty', $template_id) );
                    });
                }else{
                    ...
                    if( !empty( $cart_page_id ) ){
                        ...
                        add_action('woolentor/builder/content', function () use ( $template_id ) {
                            include_once ( $this->get_template_part('cart', $template_id) );
                        });
                    }
                }
            }elseif ( is_checkout() ){
                ...
                if( !empty($wl_checkout_page_id) ){
                    ...
                    if( !empty( is_wc_endpoint_url('order-received') ) ){
                        add_action('woolentor/builder/content', function () use ( $template_id ) {
                            include_once ( $this->get_template_part('thankyou', $template_id) );
                        });
                    }else{
                        add_action('woolentor/builder/content', function () use ( $template_id ) {
                            include_once ( $this->get_template_part('checkout', $template_id) );
                        });
                    }
                }
                ...
                if( !empty( $wl_myaccount_page_id )){
                    ...
                    add_action('woolentor/builder/content', function () use ( $template_id ) {
                        include_once ( $this->get_template_part('myaccount', $template_id ) );
                    });
                }else{
                    if( !is_user_logged_in() ){
                        ...
                        add_action('woolentor/builder/content', function () use ( $template_id ) {
                            include_once ( $this->get_template_part('myaccount/form-login',$template_id) );
                        });
                    }
                } 
            }
